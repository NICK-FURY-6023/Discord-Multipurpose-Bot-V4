...
class Verification(commands.Cog):
    ...
    if str(interaction.message.guild.id) not in data:
      ...
      if interaction.message.guild.get_role(int(data[str(interaction.message.guild.id)]["role"])) in interaction.user.roles:
        ...
        try:
          ...
          await interaction.response.send_message(content="You are now verified in this server!", ephemeral=True)
  ...
  async def verify_enable(self, ctx, channel: discord.TextChannel, role: discord.Role):
    ...
    if str(ctx.guild.id) in data:
      ...
      btn = Button(style=discord.ButtonStyle.grey, label="", emoji="<:anxTIck:1107932353654956043>", custom_id="Verify")
      view = View()
      view.add_item(btn)
      embed = discord.Embed(title='Verification', description=f"To Get Access In {ctx.guild.name}, Press the <:anxTIck:1107932353654956043> Button Below!", color=0x2f3136)
