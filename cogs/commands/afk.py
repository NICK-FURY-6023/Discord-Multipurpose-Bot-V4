...
class OnOrOff(BasicView):
    ...
    @discord.ui.button(label="Yes", emoji="<:anxTIck:1107932353654956043>", custom_id='Yes', style=discord.ButtonStyle.green)
    async def dare(self, interaction, button):
        self.value = 'Yes'
        self.stop()

    @discord.ui.button(label="No", emoji="<:anxCross:1107932430322647144>", custom_id='No', style=discord.ButtonStyle.danger)
...
class afk(commands.Cog):
    ...
    async def afk(self, ctx, *, reason=None):
        ...
        if 'discord.gg' in reason:
            emd = discord.Embed(description="<:anxAlert:1108101889687560336> | You can't Advertise in Afk Reason", color=0x0c0606)
        ...
        if '.gg/' in reason:
            eme = discord.Embed(description="<:anxAlert:1108101889687560336> | You can't Advertise in Afk Reason", color=0x0c0606)
            return await ctx.send(embed=eme)

        view = OnOrOff(ctx)
        

        em = discord.Embed(description="Should I DM You on Every Mention You Get While You're AFK?", color=0x0c0606)
        try:
        ...
        await test.delete()
        af = discord.Embed(description=f'<:anxTIck:1107932353654956043> | {ctx.author.mention} Your AFK is now set to: {reason}', color=0x0c0606)
