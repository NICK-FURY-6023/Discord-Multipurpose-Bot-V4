            ...
            if int(time.time()) > data["end_time"] and not data["ended"]:
               ...
               # ctx = data["ctx"]
                ...
                result_embed2 = discord.Embed(
                    title="<a:anxTada:1113899306148184144> {} ".format(data["prize"]),
                    color=0x030404,
                    description=f"Winner: {msg}\nHosted By: <@{host}>"
                )
                result_embed2.set_footer(icon_url=self.bot.user.display_avatar.url, text="Anxious | Giveaway Ended" if len(winners) == 1 else "Anxious | Giveaway Ended")
                ...
                result_embed.set_footer(icon_url=self.bot.user.display_avatar.url, text=f"Anxious | Giveaway Ended")
                ...
                view = View()
                view.add_item(btn)
                await giveaway_message.edit(content=f" <a:anxTada:1113899306148184144> **GIVEAWAY ENDED** <a:anxTada:1113899306148184144>", embed=result_embed2, view=view)
