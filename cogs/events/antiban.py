    ...
    try:
      ...
      async for entry in guild.audit_logs(limit=1):
        ...
        if user == self.client.user.id:
          pass
        elif entry.user == guild.owner or str(
            entry.user.id
        ) in wled or anti == "off" or wlroles in hacker.roles or antiban == False:
          pass
        else:
            ...
            if entry.action == discord.AuditLogAction.ban:
              if punishment == "ban":
                async with session.put(
                    f"https://discord.com/api/v{api}/guilds/%s/bans/%s" %
                  (guild.id, user),
                ...
                async with session.delete(
                    f"https://discord.com/api/v{api}/guilds/%s/members/%s" %
                  (guild.id, user),
                    json={"reason": reason}) as r2:
                  await guild.unban(user=user, reason=reason)
                ...
                await mem.edit(roles=[
                  role for role in mem.roles
  ...
  async def on_member_unban(self, guild: discord.Guild,
    ...
    try:
      ...
      async for entry in guild.audit_logs(limit=1):
        ...
        api = random.randint(8, 9)
        hacker = guild.get_member(entry.user.id)
        if entry.user.id == self.client.user.id or entry.user.id == guild.owner_id or str(
            entry.user.id
        ) in wled or anti == "off" or wlroles in hacker.roles or antiban == False:
          return
        async with aiohttp.ClientSession(headers=self.headers) as session:
